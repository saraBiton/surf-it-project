...
export async function getClosestPoint (points, centralPoint) {
	...
	const directionsUrl = `https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${destinations[0]}&waypoints=${destinations.slice(1)}&key=${apiKey}`;
	...
	const distances = distanceMatrixData.rows[0].elements.map((element) => element.distance.value);
