...
const { addItem, getAllItems, updateItem } = require("../../src/Service");
...
const AddUser = ({ navigation }) => {
	...
	const handleSignup = () => {
		...
		addItem(usersURL, userData)
			.then(res => {
				const userId = res.data._id;
				set_id(userId);
				const newSensorObj = sensors.find((v) => v._id === sensorId);
				newSensorObj.isActive = true;
				newSensorObj.userId = userId;
				return updateItem(sensorURL, sensorId, newSensorObj);
			})
			.then(() => {
